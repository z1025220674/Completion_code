...
李雅普诺夫指数：是用来表示两个差距极小的数值，经过n此迭代后求得最终迭代出的数值和

我们假定Lyaponuv指数用 来表示，那么对于 的不同取值有以下不同的含义：

当lambda >0时，系统运动会进入混沌状态，对应的映射叫做混沌映射；

当 lambda<0时，系统的运动状态会趋于稳定且此时对系统的初始状态不敏感，也就是说此时的映射对初始值不敏感；

当 lambda=0时，系统则处于稳定状态。

![LE氏指数计算公式](./assets/201904111551358.png)
...
Sun K H, J C Sprott. Dynamics of a simplified Lorenz system［J]. International Journal of Bifurcation and Chaos，2009, 19( 4) : 1357-1366．
...
总结：本论文最终变换公式，保证s在更大k的范围内实现混沌输出（保证Lyapunov指数大于0）；并且同时保证Lyapunov指数比SLM的值更大。
...
$$\large S[n]=0.5*\{a_0s_1[(n-z_0)\%N]+a_1s_2[(n-z_1)\%N]\}， n\in\{0,1...N-1\}$$				(3-2)
...
3. 根据a、s参数生成矩阵的行向量；
   ...
   + 存储第一行非零元素的值val，第一行非零元素的位置index(0)=
   + 根据本module参数可知矩阵的秩N；只需循环移动非零元素的位置，根据矩阵生成元素得到第n行非零元素的位置$index(n)=[index(0)+n]\%N$;**即index整体数组+n**
...
4. ##### 根据式子3-2得出，$S[z_0],S[z_1],S[z_0+N/2],S[z_1+N/2]$这四个点有值，其余均为0；此为第一列向量公式
   ...
   + 第二行三行等等公式，是根据第一行向量公式的向右移位；生成矩阵H
   + H\[0][index]
   + H\[][]
...
ofdm:是使用正交频分复用的多个子载波传递一个用户的数据，与传统子载波相比提高频谱的利用率

频分复用，叠加后可能导致峰均比局部过高的原因，就是因为频分复用后的叠加

傅里叶变换，为了将子载波的数据发送出去，需要将子载波频域信号转换成时域信号发射出去（ifft）

<img src="./image/image-20230111231341604.png" alt="image-20230111231341604" />
...
https://www.bilibili.com/video/BV1G741197Tt/?spm_id_from=333.337.search-card.all.click&vd_source=82e216ecd7f8f5f99d6fe7a813a2a5d1

链接主题：关于多址技术的讲解，并且介绍了单位

实际项目：一个ofdm符号进行4096点ifft，子载波数量是4096，相当于4096个数据通道，数据通道的每拍数据不同
